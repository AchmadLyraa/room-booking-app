...
export default function FoodsSnacksManagementClient({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create new item
      const result = await createFood(formData.name);
  ...
  const handleDelete = async (itemId: string) => {
    ...
    try {
      ...
      showSuccess(`${"Makanan"} berhasil dihapus`);
    } catch (error) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      setItemList((prev) =>
        prev.filter(
          (item) =>
            !selectedItems.includes(item.id) || failedItems.includes(item.id),
        ),
      );
      setSelectedItems([]);
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 md:p-8">
          <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <h1 className="text-2xl font-bold uppercase">MAKANAN</h1>
