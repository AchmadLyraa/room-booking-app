...
export function RoomAvailabilityTable() {
  ...
  const [displayTime, setDisplayTime] = useState(new Date()); // State terpisah untuk jam
  ...
  useEffect(() => {
    ...
    }, 1000); // Update tiap detik biar real-time
  ...
  const timeFormatted = format(displayTime, "HH:mm"); // Pakai displayTime, bukan currentDate!

  // Fix untuk disabled date - bikin const di luar biar gak re-create terus
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex border-3 border-black brutal-shadow overflow-hidden w-full md:w-auto self-end md:self-auto">
          <div className="flex-1 px-3 md:px-4 py-2 bg-white text-black font-bold text-sm md:text-base text-center border-r-3 border-black">
            {timeFormatted}
          </div>
          ...
          <Popover>
            ...
            <PopoverContent
              className="w-auto p-0 border-3 border-black brutal-shadow bg-white"
              align="end"
            >
              <Calendar
                mode="single"
                ...
                disabled={(date) => date < today} // Pakai const today yang udah di-set
                className="border-0"
              />
            </PopoverContent>
          </Popover>
        ...
        <table className="w-full min-w-[800px]">
          <thead>
            <tr className="bg-[#f5f5f5] border-b-3 border-black">
              ...
              <th className="px-4 py-3 text-left font-bold uppercase text-sm border-r-2 border-black">
                RUANGAN
              </th>
              <th className="px-4 py-3 text-center font-bold uppercase text-sm border-r-2 border-black">
                KAPASITAS
          ...
          <tbody>
            {availability.map((room, index) => {
              ...
              return (
                ...
                >
                  <td className="px-4 py-4 text-center font-bold border-r-2 border-black">
                    {index + 1}
                  </td>
                  <td className="px-4 py-4 font-bold border-r-2 border-black">
                    {room.name}
                  </td>
                  <td className="px-4 py-4 text-center font-bold border-r-2 border-black">
                    {room.capacity}
      ...
      <div className="md:hidden space-y-4">
        {availability.map((room, index) => {
          ...
          return (
            <div
              key={room.id}
              className="bg-white border-3 border-black brutal-shadow p-4"
            >
              {/* Room Header */}
